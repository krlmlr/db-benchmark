...
source("./helpers.R")
source("./duckplyr/helpers-duckplyr.R")

src_x = Sys.getenv("SRC_X", NA_character_)

# if (get.nrow(src_x) > 1e9L) {
#   cat("# sort with duckplyr skipped due data volume cap for single machine set to total 1e9 rows")
...
suppressPackageStartupMessages(library(duckplyr, warn.conflicts=FALSE))
